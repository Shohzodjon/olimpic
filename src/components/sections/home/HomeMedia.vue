    ...
    <section class="home-media">
        <div class="container">
            <div class="home-media__flex">
                <h2 class="home-media__title">{{ $t('mediaFile') }}</h2>
                <div class="custom-tab">
                    <button @click="tabClick('photo')"
                        :class="[currentTab == 'photo' ? 'active-tab' : '']">{{ $t('photoGallery') }}</button>
                    <button @click="tabClick('video')"
                        :class="[currentTab != 'photo' ? 'active-tab' : '']">{{ $t('videoGallery') }}</button>
                </div>
            </div>
            <div v-if="isLoad">
